        ...
        try{
            if(!data.email || !data.doctorId || !data.timeType || !data.date){
                ...
                let user = await db.User.findOrCreate({
                  ...
                  console.log('eminh check user: ', user[0])
                  //tạo thông tin đặt lịch
                  if (user && user[0]){
        ...
        try{
            if(!data.token || !data.doctorId ){
                ...
                if(appointment){
                    ...
                    resolve({
                        errCode: 0,
                        errMessage: "Update appointment succedd"
                    })
                }else{
                    resolve({
                        errCode: 2,
                        errMessage: "Appointment has been acitvated or does not exits"
                    })
                }
